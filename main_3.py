...
"""
# Currently: Trying fix pelvis to turn tgt
...
class MotionConverter:
    ...
    def convert_to_qpos(self, mocap_data, current_qpos):
        ...
        # 2. Root Rotation (FIXED)
        # We must apply the Unity->MuJoCo axis swap to the Root Quaternion too.
        # Mapping from config.py: Unity Y(1) -> MuJoCo Z(2) is the critical fix for Yaw.
        
        if self.is_calibrated:
            # Calibrated: Relative to T-Pose
            root_quat = pose24[0]
            neutral_root = self.neutral_pose[0]
            rel_quat = self.compute_relative_rotation(root_quat, neutral_root)
            
            # [FIX] Remap axes:
            # MuJoCo W (3) = Unity W (3)
            # MuJoCo X (4) = Unity Z (2)
            # MuJoCo Y (5) = -Unity X (0)
            # MuJoCo Z (6) = Unity Y (1)  <-- This fixes the "Turn = Pitch" bug
            ...
            # Uncalibrated: Use raw rotation
            raw_quat = pose24[0]
            raw_quat = normalize(np.array(raw_quat))
